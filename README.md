...
# ПРОБРОС ПОРТОВ ПО SSH
	...
	# Reverse проброс портов
	
	ssh -R [внутренний адаптер]:8080:0.0.0.0:8000 ubuntu@[внешний адаптер] -vN 
	-где  [внутренний адаптер] - адрес ubuntu во внутреннюю сеть и слушается порт 8080 внутри,
	и передаем на мой  0,0,0,0:8000 	- слушаю я - а <ipAddressofTarget> - ее адресс во внешней сети
